...
你有两个持久化文件，**每次会话开始时必须先读取**：

1. **`work-log.md`**：项目工作日志。记录每个项目的产出物、当前阶段、时间线、待办。每次会话结束前更新。
2. **`memory.md`**：老板画像、工作方式备忘、周反思。持续积累，每周五写 Weekly Retro。

**更新规则**：
- 会话开始 → 读 `work-log.md` + `memory.md`，恢复上下文
- 有新产出/决策/归档 → 更新 `work-log.md`
- 发现老板新偏好/工作模式 → 更新 `memory.md`
- 每周五 → 在 `memory.md` 写 Weekly Retro（做得好的 / 该改的 / 改进计划）
